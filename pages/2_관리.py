        ...
        if selected_backup_sheet:
            try:
                ...
                if len(all_values) < 2:
                    ...
                    # 중복 체크는 세션 데이터(메인 시트와 동기화됨)로 — 매 렌더 전체 재읽기 방지
                    current_data = st.session_state.container_list
...
with st.container(border=True):
    ...
    if spreadsheet_preview:
        ...
        all_sheet_titles = all_worksheet_titles
        target_daily = [
...
if spreadsheet_move:
    all_sheets_move = all_worksheet_titles
