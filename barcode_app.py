...
def save_data_to_storage():
    """현재 st.session_state.container_list를 브라우저 저장소에 저장합니다."""
    list_to_save = []
...
def load_data_from_storage():
    """브라우저 저장소에서 데이터를 불러와 st.session_state를 초기화합니다."""
...
with st.expander("⬆️ (필요시 사용) 백업 파일로 데이터 복구/일괄 등록"):
    ...
    uploaded_file = st.file_uploader("백업된 엑셀(xlsx) 파일을 업로드하세요.", type=['xlsx'], on_change=reset_upload_state)
